document.addEventListener('DOMContentLoaded', function() {
    ...
    btnStats.addEventListener('click', () =>{
        ...
        getTasks(boardId).then(() => {
            ...
            let categoryToDo = tasksData.filter(task => task.idStatus == 1).length;
            let categoryInProgress = tasksData.filter(task => task.idStatus == 2).length;
            let categoryDone = tasksData.filter(task => task.idStatus == 3).length;

            let valueA = ["Do zrobienia", "W trakcie", "Zrobione"];
            let valueB = [categoryToDo, categoryInProgress, categoryDone];
